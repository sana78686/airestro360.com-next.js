...
export async function generateMetadata({
  ...
  const page = getMarketingPage(tool, 'id')
...
export default async function IdMarketingToolPage({ params }: { params: Promise<{ tool: string }> }) {
  const { tool } = await params
  const key = tool?.toLowerCase() || ''
  if (!key || RESERVED.has(key) || !isMarketingSlug(key)) notFound()
  const page = getMarketingPage(key, 'id')
  if (!page) notFound()
