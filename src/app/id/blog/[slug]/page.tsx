...
export async function generateMetadata({
  ...
  try {
    const data = (await getBlogBySlug(slug, 'id')) as Record<string, string | undefined>
    ...
    return {
      title,
      description,
      alternates: { canonical: `/id/blog/${encodeURIComponent(slug)}` },
      openGraph: {
        title,
        description,
        type: 'article',
        url: `/id/blog/${encodeURIComponent(slug)}`,
        locale: 'id_ID',
...
export default async function IdBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  let data: Record<string, unknown>
  try {
    data = (await getBlogBySlug(slug, 'id')) as Record<string, unknown>
  } catch {
    notFound()
  }
