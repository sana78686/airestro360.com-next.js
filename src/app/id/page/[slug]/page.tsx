...
export async function generateMetadata({
  ...
  try {
    const data = (await getPageBySlug(slug, 'id')) as Record<string, string | undefined>
...
export default async function IdCmsDynamicPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  let data: Record<string, unknown>
  try {
    data = (await getPageBySlug(slug, 'id')) as Record<string, unknown>
  } catch {
    notFound()
  }
