...
/**
 * Same contract as CMS PHP sync: push SEO files to the frontend.
 * Body: JSON `{ secret, action, content }` or `multipart/form-data` with `secret`, `action`, `content`.
 *
 * Set `CMS_SEO_SYNC_SECRET` on the Next server. Must match `FRONTEND_SEO_SYNC_SECRET` in CMS .env.
 */
